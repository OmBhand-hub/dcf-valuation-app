...
def _latest_value(series):
    if series is None:
        return None
    try:
        # yfinance statements have most recent period in the first column
    ...
    try:
        ...
        try:
            ...
            if balance_sheet is not None and not balance_sheet.empty:
                # Try direct "Total Debt" if Yahoo exposes it
                total_debt_direct = _find_row_case_insensitive(balance_sheet, ["Total Debt"])

                # Build total debt from parts when direct isn't available
                ...
                if total_debt_direct is not None:
                    ...
                    if parts:
                        ...
                        # Fallback (not ideal for WACC, but better than always 0)
        ...
        # Optional: debug the balance sheet row names
...
discount_rates = [0.08, 0.09, 0.10, 0.11, 0.12]  # as decimals
...
for g in growth_rates:
    ...
    for r in discount_rates:
        ...
        for i in range(years):
            intrinsic += fcf / ((1 + r) ** (i + 1))  # simple constant FCF stream
        try:
